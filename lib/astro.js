...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "𝔸ℝ𝕋ℍ𝕌ℝ-𝕄𝔻"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "𝔸ℝ𝕋ℍ𝕌ℝ-𝕄𝔻") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "𝔸ℝ𝕋ℍ𝕌ℝ-𝕄𝔻") => {
...
app.listen(randomPort, () =>
  console.log(
    "𝔸ℝ𝕋ℍ𝕌ℝ-𝕄𝔻 Server listening on http://localhost:" + randomPort + "/ "
  )
);
